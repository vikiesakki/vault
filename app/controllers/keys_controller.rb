class KeysController < ApplicationController
  ...
  before_action :find_project_by_project_id, except: :public_keys
  before_action :validate_public_key, only: :public_keys
  before_action :authorize, except: :public_keys
  ...
  def public_keys
    ...
    @keys = Vault::Key.where(is_public: 1)
    if @query
      if @query.match(/#/)
        tag_string = (@query.match(/(#)([^,]+)/))[2]
        tag = Vault::Tag.find_by_name(tag_string)
        ...
        if params[:search_fild] == 'name'
          @keys = @keys.where(name: @query)
        elsif params[:search_fild] == 'url'
          @keys = @keys.where(url: @query)
        elsif params[:search_fild] == 'tag'
          tag = Vault::Tag.find_by_name(@query)
          @keys = tag.nil? ? nil : tag.keys.where(id: @keys)
        end
      end
    end
  ...
  def key_params
    params.require(:vault_key).permit(:type, :is_public, :name, :body, :login, :file, :url, :comment, :tags)
  end
